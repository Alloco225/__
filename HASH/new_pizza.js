...
function solve(){
    ...
    fs.readFile(file, (err, data)=>{
            ...
            $coming_clients = 0;
            let best_client_count = 0;
            let more_disliked_ingredient = null;
            let more_disliked_ingredient_index = 0;
            let _ingredients = $ingredients.slice();
            
            
            let best_score_found = false;
            let counter = 0;
            // while (!best_score_found && counter < 10000) {
            //     counter++;
            //     console.log(">>", counter)
            for(let i = 0; i< $ingredients.length; i++){
                    _ingredients = $ingredients.slice();
    
                    let _result = _ingredients.map((ing) => ing.name);
    
                    console.log("\n\n!!!! INGREDIENTS : ", i, _result);
                    $clients.forEach(client => {
                        if(client.isComing(_result.slice(1))){
                            $coming_clients ++;
                        }
                    });
    
                    more_disliked_ingredient = $ingredients[0];
                    more_disliked_ingredient_index = 0;
                    for(var y = 0; y < $ingredients.length -1; y++){
                        if(more_disliked_ingredient.disliked < $ingredients[y].disliked){
                            more_disliked_ingredient = $ingredients[y];
                            more_disliked_ingredient_index = y;
                        }
                    }
    
                    
                    console.log("\n!!!! CLIENTS :", $coming_clients);

                    if($coming_clients > best_client_count){
                        best_client_count = $coming_clients;
                    }else{
                        console.log("\ Best score :", best_client_count);

                        best_score_found = true;
                        break;
                    }
                    // Remove from list
                    $ingredients.splice(more_disliked_ingredient_index, 1);
                    // 2nd list
                }
            // }
